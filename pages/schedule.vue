          ...
          <v-card-actions>
            <v-tooltip top>
              <template #activator="{ on, attrs }">
                <v-btn icon class="ml-6 mb-2" v-bind="attrs" @click="addValue" v-on="on">
                    ...
                    <v-container>
                      <v-row>
                        <v-col>
                          ...
                          <v-card-text>
                            <v-select v-model="startTime" dense :items="startTimeChoices"></v-select>
                            <v-select v-model="endTime" dense :items="endTimeChoices"></v-select>
                          </v-card-text>
                        </v-col>
                      </v-row>
                      <v-divider></v-divider>
                      <v-row>
                        <v-col>
                          <v-subheader> 選択ステップ </v-subheader>
                          <v-card-text>
                            <v-select v-model="stepMinutes" dense :items="stepMinutesChoices"></v-select>
                          </v-card-text>
                        </v-col>
                      </v-row>
                    </v-container>
    ...
    schedule() {
      return Object.keys(this.formattedValues)
        .map((date) => {
          ...
          const formattedTime = this.timeListFormatter(this.formattedValues[date])
