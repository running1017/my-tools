...
export default {
    ...
    computed: {
        ...
        timeLabels() {
            const length =
                ((this.endTime - this.startTime) / this.stepMinutes) * 60;
        ...
        datetime() {
            ...
            const startTime = this.timeFormatter(this.timeRange[0]);
            const endTime = this.timeFormatter(this.timeRange[1]);
            return `${month}/${date}（${day}）${startTime}～${endTime}`;
        },
    },
    methods: {
        timeFormatter(time) {
